...
import { NaverLandNewsSampleClient } from "@/components/mockup/NaverLandNewsSampleClient";

export const metadata: Metadata = {
  title: "디자인 목업 | 네이버부동산 리스트형",
  robots: { index: false, follow: false },
};

export default function NaverLandNewsMockupPage() {
  return (
    <LandingShell>
      <div className="border-b border-[#d450ff]/30 bg-[#120818] px-4 py-2 text-center text-xs text-[#e9d5ff]">
        샘플 · 기존 리스트 유지 + 썸네일·제목·요약 · 홈 미적용{" "}
        <Link href="/" className="text-[#f0abfc] hover:underline">
          ← 홈
        </Link>
        {" · "}
        <Link href="/news?source=naver" className="text-white/40 hover:underline">
          현재 /news
        </Link>
      </div>
      <LandingHeader />
