...
import { ChungnamNewsSampleClient } from "@/components/mockup/ChungnamNewsSampleClient";

export const metadata: Metadata = {
  title: "디자인 목업 | 충남도청 최근소식",
  robots: { index: false, follow: false },
};

export default function ChungnamNewsMockupPage() {
  return (
    <LandingShell>
      <div className="border-b border-[#d450ff]/30 bg-[#120818] px-4 py-2 text-center text-xs text-[#e9d5ff]">
        샘플 · 충남도청 최근소식 · 공지 제외 · 키워드·제목·내용(첫 줄)·원문 (사진 없음){" "}
        <Link href="/" className="text-[#f0abfc] hover:underline">
          ← 홈
        </Link>
        {" · "}
        <Link href="/mockup/molit-news" className="text-white/40 hover:underline">
          국토부 샘플
        </Link>
        {" · "}
        <Link href="/news?source=chungnam" className="text-white/40 hover:underline">
          현재 /news
        </Link>
      </div>
      <LandingHeader />
