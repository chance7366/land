...
import type { AdminDashboardPayload } from "@/lib/supabase/repos/dashboard-stats";
import { ACCENT } from "@/lib/mockup/admin-visual-dashboard-sample";
...
function ResponseTacho({
  ...
  const pct = Math.min(1, hours / Math.max(target, 0.1));
  ...
  const status = hours <= target * 0.5 ? "양호" : hours <= target ? "보통" : "지연";
  const statusColor =
...
function FunnelVisual({ funnel }: { funnel: AdminDashboardPayload["funnel"] }) {
  if (!funnel.length) {
    return (
      <p className="py-10 text-center text-sm text-white/40">
        페이지뷰·클릭 이벤트가 쌓이면 깔때기가 표시됩니다.
      </p>
    );
  }
  return (
    <div className="flex flex-col items-center gap-2 py-2">
      {funnel.map((row, i) => {
        ...
        const drop =
          i === 0 ? null : Math.round((1 - row.value / Math.max(funnel[i - 1].value, 1)) * 100);
        return (
...
function EmptyHint({ text }: { text: string }) {
  return <p className="py-12 text-center text-sm text-white/35">{text}</p>;
}

function formatConsultWhen(row: AdminDashboardPayload["action"]["consultRecent"][number]) {
  if (row.preferredAt) return row.preferredAt;
  try {
...
export function AdminDashboardView({ data }: { data: AdminDashboardPayload }) {
  ...
  const topPropChart = data.topProperties.map((r) => ({ name: r.name, views: r.value }));
  const dwellChart = data.dwellTop.map((r) => ({ name: r.name, dwellSec: r.value }));
  const maxAuction = Math.max(1, ...data.topAuctions.map((r) => r.value));

  return (
    <div className="relative isolate min-h-screen overflow-hidden text-landing-text">
      ...
      <div className="relative z-10 mx-auto max-w-[1400px] space-y-12 px-4 py-8 md:px-8 md:py-10">
        <header className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-white/50">
              DB 집계 · 행동 이벤트{data.eventsReady ? " 연동됨" : " 대기 중(테이블/트래픽 필요)"}
            </p>
          </div>
        ...
        <section>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <AccentCard
              accentRgb={action.legalOver24h > 0 ? "248, 113, 113" : ACCENT.greenRgb}
              accentHex={action.legalOver24h > 0 ? "#f87171" : ACCENT.green}
              className="relative"
            >
              {action.legalOver24h > 0 ? (
                <span className="absolute right-4 top-4 flex h-3 w-3">
                  <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-red-400 opacity-75" />
                  <span className="relative inline-flex h-3 w-3 rounded-full bg-red-500" />
                </span>
              ) : null}
              <Link href="/admin/legal" className="block">
                ...
                <p className="mt-2 text-xs text-red-200/80">
                  24시간 초과 <strong>{action.legalOver24h}</strong>건 · 즉시 답변 권장
                </p>
              </Link>
            </AccentCard>
            ...
            <AccentCard accentRgb={ACCENT.goldRgb} accentHex={ACCENT.gold}>
              <Link href="/admin/consultations" className="block">
                ...
                <p className="mt-3 font-mono text-4xl font-extrabold text-white">
                  {action.consultPending}
                  <span className="ml-1 text-base font-medium text-white/45">건</span>
                </p>
                <ul className="mt-3 space-y-1.5 border-t border-white/10 pt-3">
                  {action.consultRecent.length ? (
                    action.consultRecent.slice(0, 4).map((row) => (
                      <li
                        key={row.id}
                        className="flex items-center justify-between gap-2 text-xs text-white/70"
                      >
                        <span className="shrink-0 font-mono" style={{ color: ACCENT.gold }}>
                          {formatConsultWhen(row)}
                        </span>
                        <span className="truncate text-right">
                          {row.clientName} · {row.category}
                        </span>
                      </li>
                    ))
                  ) : (
                    <li className="text-xs text-white/40">대기 중인 예약이 없습니다</li>
                  )}
                </ul>
              </Link>
            </AccentCard>
        ...
        <section>
          <ZoneTitle step="Zone 2" title="매물 & 경매 머니 맵" subtitle="등록 현황 · 클릭이 몰리는 물건" />
          <div className="grid gap-4 lg:grid-cols-12">
            ...
            <AccentCard accentRgb={ACCENT.blueRgb} accentHex={ACCENT.blue} className="lg:col-span-4">
              ...
              <div className="mt-2 h-64">
                {topPropChart.length ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={topPropChart} layout="vertical" margin={{ left: 4, right: 8 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.06)" />
                      <XAxis type="number" tick={{ fill: "#94a3b8", fontSize: 10 }} />
                      <YAxis type="category" dataKey="name" width={100} tick={{ fill: "#cbd5e1", fontSize: 10 }} />
                      <Tooltip contentStyle={tipStyle} />
                      <Bar dataKey="views" name="클릭" fill={ACCENT.blue} radius={[0, 6, 6, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                ) : (
                  <EmptyHint text="매물 클릭 이벤트가 쌓이면 TOP5가 표시됩니다." />
                )}
              </div>
            </AccentCard>
            ...
            <AccentCard accentRgb={ACCENT.goldRgb} accentHex={ACCENT.gold} className="lg:col-span-4">
              ...
              {data.topAuctions.length ? (
                <ul className="mt-3 space-y-2">
                  {data.topAuctions.map((row, i) => {
                    const w = Math.round((row.value / maxAuction) * 100);
                    return (
                      <li key={row.name} className="rounded-xl border border-white/10 bg-black/25 p-2.5">
                        <div className="mb-1 flex justify-between text-xs">
                          ...
                          <span className="font-mono text-white/50">{row.value}</span>
                        </div>
                        ...
                        {row.extra != null ? (
                          <p className="mt-1 text-[10px] text-white/40">알림 매칭 {row.extra}건</p>
                        ) : null}
                      </li>
                    );
                  })}
                ...
                <EmptyHint text="경매 클릭 이벤트가 쌓이면 랭킹이 표시됩니다." />
              )}
            </AccentCard>
          </div>
        </section>
        ...
        <section>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <AccentCard accentRgb="249, 115, 22" accentHex={ACCENT.orange}>
              ...
              <div className="h-60">
                {data.menuShare.length ? (
                  ...
                  <EmptyHint text="page_view 수집 후 메뉴 점유율이 표시됩니다." />
                )}
              </div>
            </AccentCard>
            <AccentCard accentRgb={ACCENT.violetRgb} accentHex={ACCENT.violet}>
              ...
              <FunnelVisual funnel={data.funnel} />
            </AccentCard>
          </div>
        </section>
        ...
        <section>
          ...
          <AccentCard accentRgb={ACCENT.blueRgb} accentHex={ACCENT.blue}>
            <div className="mb-3 flex flex-wrap gap-4 text-[11px] text-white/50">
              ...
              <span className="flex items-center gap-1.5">
                <span className="h-0.5 w-4" style={{ background: ACCENT.orange }} /> 방문자(PV)
              </span>
              {data.storyMarkers.length ? (
                <span className="text-amber-200/70">
                  {data.storyMarkers.map((m) => `${m.month} ${m.label}`).join(" · ")}
                </span>
              ) : null}
            </div>
            <div className="h-72">
              {data.monthly.length ? (
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={data.monthly}>
                    <defs>
                      <linearGradient id="consultFillLive" x1="0" y1="0" x2="0" y2="1">
                ...
                <EmptyHint text="상담·방문 데이터가 쌓이면 추이가 표시됩니다." />
              )}
            </div>
          </AccentCard>
        </section>
        ...
        <section>
          <ZoneTitle
            step="Zone 5 · Sales"
            title="고의도 고객 행동"
            subtitle="30초+ 체류 · 공유/복사/인쇄 액션"
          />
          <div className="grid gap-4 lg:grid-cols-12">
            <AccentCard accentRgb={ACCENT.greenRgb} accentHex={ACCENT.green} className="lg:col-span-4">
              <h3 className="mb-3 text-sm font-bold" style={{ color: ACCENT.green }}>
                고객 액션 발생 현황
              </h3>
              <div className="space-y-3">
                {[
                  { icon: Share2, label: "공유", value: data.customerActions.share },
                  { icon: Copy, label: "링크 복사", value: data.customerActions.copyLink },
                  { icon: Printer, label: "프린트", value: data.customerActions.print },
                ].map((row) => (
                  <div
                    key={row.label}
            ...
            <AccentCard accentRgb={ACCENT.blueRgb} accentHex={ACCENT.blue} className="lg:col-span-8">
              ...
              <div className="h-64">
                {dwellChart.length ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={dwellChart} layout="vertical" margin={{ left: 4, right: 8 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.06)" />
                      <XAxis type="number" tick={{ fill: "#94a3b8", fontSize: 10 }} />
                      <YAxis
                        type="category"
                  ...
                  <EmptyHint text="상세 30초+ 체류 이벤트가 쌓이면 TOP5가 표시됩니다." />
                )}
              </div>
            </AccentCard>
          </div>
        ...
        <section>
          ...
          <div className="grid gap-4 lg:grid-cols-12">
            <AccentCard accentRgb={ACCENT.goldRgb} accentHex={ACCENT.gold} className="lg:col-span-5">
              ...
              <div className="h-56">
                {data.searchKeywords.length ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={data.searchKeywords}>
                      <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.06)" />
                  ...
                  <EmptyHint text="검색 이벤트가 쌓이면 키워드가 표시됩니다." />
                )}
              </div>
            </AccentCard>
            ...
            <AccentCard accentRgb="248, 113, 113" accentHex="#f87171" className="lg:col-span-7">
              ...
              {data.supplyGap.length ? (
                <ul className="space-y-2.5">
                  {data.supplyGap.map((row, i) => (
                    <li
                      key={row.region}
                ...
                <EmptyHint text="검색·등록 데이터가 쌓이면 공급 갭이 표시됩니다." />
              )}
            </AccentCard>
          </div>
        </section>
        ...
        <section>
          ...
          <div className="grid gap-4 lg:grid-cols-12">
            <AccentCard accentRgb={ACCENT.violetRgb} accentHex={ACCENT.violet} className="lg:col-span-7">
              ...
              <p className="mb-2 text-[11px] text-white/40">?ref=youtube · blog · direct</p>
              <div className="h-64">
                {data.attribution.length ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={data.attribution}>
                      <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.06)" />
                  ...
                  <EmptyHint text="?ref= 유입 + 전환 이벤트가 쌓이면 표시됩니다." />
                )}
              </div>
            </AccentCard>
            ...
            <AccentCard accentRgb={ACCENT.greenRgb} accentHex={ACCENT.green} className="lg:col-span-5">
              ...
              <div className="grid grid-cols-2 gap-2">
                <ResponseTacho
                  hours={data.responseHours.legalAvgHours}
                  target={data.responseHours.targetHours}
                  label="찬스상담소"
                  accent={ACCENT.green}
                />
                <ResponseTacho
                  hours={data.responseHours.consultAvgHours}
                  target={data.responseHours.targetHours}
                  label="상담 예약"
                  accent={ACCENT.blue}
                />
