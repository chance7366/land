...
function useGeminiToday(pathname: string) {
...
export function AdminSidebar({ authEnabled }: AdminSidebarProps) {
  ...
  const geminiToday = useGeminiToday(pathname);
  ...
  return (
    <>
      {/* 데스크톱: 좌측 사이드바 */}
      <aside className="fixed left-0 top-0 z-40 hidden h-full w-56 flex-col border-r border-white/10 bg-white/[0.04] py-6 shadow-[0_8px_32px_rgba(0,0,0,0.35)] backdrop-blur-xl md:flex">
      ...
      {/* 모바일: 상단 얇은 헤더 */}
      <header className="sticky top-0 z-40 border-b border-white/10 bg-landing-bg/90 px-3 py-2.5 backdrop-blur-xl md:hidden">
        <div className="flex items-center justify-between gap-2">
          <div className="min-w-0">
            <div className="flex items-center gap-1.5">
              <BarChart3 className="h-4 w-4 shrink-0 text-blue-400" aria-hidden />
              <h1 className="truncate text-sm font-bold text-landing-text">찬스 관리자</h1>
              {!authEnabled && (
                <span className="shrink-0 rounded bg-violet-500/20 px-1.5 py-0.5 text-[9px] font-bold text-violet-300">
                  개발
                </span>
              )}
            </div>
          ...
          <div className="flex shrink-0 items-center gap-2">
            <Link
              href="/"
              className="rounded-lg border border-white/10 px-2 py-1 text-[11px] text-landing-muted hover:text-blue-400"
            >
              사용자
            </Link>
            {authEnabled ? (
      ...
      {/* 모바일: 하단 네비 (사용자 페이지 UserBottomNav와 동일 패턴) */}
      <nav
      ...
      >
        <div className="flex items-stretch gap-0 overflow-x-auto overscroll-x-contain px-1 py-1.5 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {NAV.map((item) => {
            const active = isNavActive(pathname, item.href);
            const Icon = item.Icon;
            return (
