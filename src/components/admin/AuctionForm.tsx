...
function locNotesFromRights(text: string): LocationAnalysisNotes {
  return parseLocationAnalysisNotes(sectionFromRights(text, "입지분석텍스트JSON"));
}
...
function buildRightsAnalysis(
  ...
  const lines = [
    ...
    bidNotes && hasBiddingValuationNotes(bidNotes)
      ...
      : "",
    locNotes && hasLocationAnalysisNotes(locNotes)
      ? `[입지분석텍스트JSON]\n${stringifyLocationAnalysisNotes(locNotes)}`
      : "",
    hasMeta ? `[등록메타JSON]\n${JSON.stringify(meta)}` : "",
  ].filter(Boolean);
  return lines.join("\n\n");
...
export function AuctionForm({ initial }: AuctionFormProps) {
  ...
  const locUploadRef = useRef<HTMLInputElement>(null);
  ...
  const [locSlotUploading, setLocSlotUploading] = useState<LocationAnalysisDocType | null>(null);
  const [pendingLocType, setPendingLocType] = useState<LocationAnalysisDocType | null>(null);
  ...
  async function uploadLocAnalysisSlot(
    type: LocationAnalysisDocType,
    fileList: FileList | null,
  ) {
    if (!fileList?.length) return;
    setLocSlotUploading(type);
    setError("");
    try {
      const existing = filesForSlot(attachments, type);
      const remainingSlot = MAX_ATTACHMENTS_PER_SLOT - existing.length;
      ...
      const data = (await res.json()) as { urls?: string[]; error?: string };
      if (!res.ok) {
        setError(data.error ?? "입지분석 자료 업로드에 실패했습니다.");
        return;
      }
      const urls = data.urls ?? [];
      const added: AuctionAttachment[] = urls.map((url, i) => ({
      ...
      if (!added.length) return;
      setAttachments((prev) => [...prev, ...added].slice(0, MAX_ATTACHMENTS_TOTAL));
      setToast(`입지분석 자료 ${added.length}건이 첨부되었습니다.`);
    } catch {
      setError("입지분석 자료 업로드 중 오류가 발생했습니다.");
    } finally {
      setLocSlotUploading(null);
      if (locUploadRef.current) locUploadRef.current.value = "";
    }
  }
    ...
    <div className="mx-auto max-w-5xl pb-28 font-[family-name:var(--font-unifine),Outfit,sans-serif] text-slate-200">
      ...
      <div className="mt-6 space-y-5">
        ...
        <Section
          n={9}
          title="입지분석"
          hint="슬롯별 파일 다건 첨부 + 텍스트 직접 입력/붙여넣기 가능 · PDF/PNG/JPG · TXT · Excel · CSV · Word · 한글(hwp/hwpx). 회원리포트 §4 입지분석 시 우선 참조합니다."
        >
          <input
            ref={locUploadRef}
            type="file"
            ...
            className="hidden"
            onChange={(e) => {
              if (pendingLocType) void uploadLocAnalysisSlot(pendingLocType, e.target.files);
              setPendingLocType(null);
            }}
          />
          <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
            {LOCATION_ANALYSIS_SLOTS.map((slot) => {
              const files = filesForSlot(attachments, slot.type);
              const note = locNotes[slot.type] ?? "";
              const atLimit = files.length >= MAX_ATTACHMENTS_PER_SLOT;
              const hasContent = files.length > 0 || Boolean(note.trim());
              return (
                <div key={slot.type} className="rounded-xl border border-white/10 bg-black/25 p-4">
                  ...
                  <label className="mt-3 block">
                    ...
                    <textarea
                      ...
                      value={note}
                      onChange={(e) =>
                        setLocNotes((prev) => ({ ...prev, [slot.type]: e.target.value }))
                      }
                      placeholder={slot.placeholder}
                    />
                    <span className="mt-0.5 block text-right text-[10px] text-slate-600">
                  ...
                  <div className="mt-2">
                    <button
                      type="button"
                      disabled={locSlotUploading === slot.type || atLimit}
                      onClick={() => {
                        setPendingLocType(slot.type);
                        locUploadRef.current?.click();
                      }}
                      className="w-full rounded-lg border border-white/10 py-1.5 text-[11px] text-slate-200 hover:bg-white/5 disabled:opacity-40"
                    >
                      {locSlotUploading === slot.type
                        ? "업로드…"
                        : atLimit
                          ? "파일 슬롯 한도 도달"
                          : files.length
        ...
        <Section n={11} title="찬스부동산 의견" hint="고객 안내용 · 권리분석과 별도 저장(memo)">
          <textarea
                ...
                [
                  ...
                  },
                  {
                    ...
                    sections: [
                      ...
                      "입지분석(상세) · 수익률·명도·출구·최종 결론 대기 (4~7)",
                    ],
