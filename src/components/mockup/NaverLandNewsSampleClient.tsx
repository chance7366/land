...
import {
  MOCK_NAVER_LAND_NEWS,
  matchNaverKeywords,
} from "@/lib/mockup/naver-land-news-sample";
...
/**
 * 샘플: 기존 /news 리스트 셸 유지 + 네이버형(썸네일 | 제목 / 요약 / 언론사)
 */
export function NaverLandNewsSampleClient() {
  const [keywords, setKeywords] = useState<string[]>([]);
  const [draft, setDraft] = useState("");

  const filtered = useMemo(
    () =>
      MOCK_NAVER_LAND_NEWS.filter((row) =>
        matchNaverKeywords(row.title, row.summary, keywords),
      ),
    [keywords],
  );
    ...
    <div className="mx-auto flex max-w-6xl flex-col gap-6 px-container-padding-mobile py-10 md:flex-row md:px-8 md:py-14">
      ...
      <aside className="w-full shrink-0 md:w-60 lg:w-64">
        <GlassCard className="sticky top-24 p-3">
          ...
          <nav className="space-y-1" aria-label="뉴스 출처">
            {[
              { label: "전체", count: 16, active: false },
              { label: "네이버부동산뉴스", count: filtered.length, active: true },
              { label: "국토교통부", count: 3, active: false },
              { label: "충남도청", count: 2, active: false },
              { label: "홍성군청", count: 2, active: false },
              { label: "부동산테크", count: 2, active: false },
              { label: "부동산114", count: 2, active: false },
            ].map((item) => (
              <div
                key={item.label}
              ...
              >
                <Newspaper className="h-4 w-4 shrink-0 opacity-80" aria-hidden />
      ...
      <div className="min-w-0 flex-1">
        <header className="mb-6 border-b border-white/10 pb-6">
          ...
          <h1 className="mt-1 text-3xl font-extrabold text-white md:text-4xl">네이버부동산뉴스</h1>
          <p className="mt-2 text-sm text-white/60">
            기존 리스트 레이아웃을 유지하고, 썸네일·제목·요약(네이버형)을 넣었습니다.
          </p>
        </header>
          ...
          <div className="mt-2 flex gap-2">
            <div className="relative min-w-0 flex-1">
              ...
              <input
                ...
                placeholder="예: 내포, 경매, 전세 — Enter로 추가"
                value={draft}
              ...
              {filtered.map((row) => (
                <li key={row.id}>
                  <div className="flex flex-col gap-3 px-4 py-4 transition hover:bg-white/[0.03] sm:flex-row sm:items-center sm:gap-3">
                    <span className="inline-flex h-6 shrink-0 items-center whitespace-nowrap rounded-full border border-emerald-400/40 bg-emerald-500/15 px-2.5 text-[11px] font-bold leading-none text-emerald-300">
                      네이버
                    </span>
                    <button
                      type="button"
        ...
        <p className="mt-4 text-center text-[11px] text-white/40">
          샘플 · 기존 리스트 + 네이버형 썸네일/요약 ·{" "}
          <Link href="/news?source=naver" className="text-[#e9d5ff]/80 hover:underline">
            현재 /news
          </Link>
        </p>
      </div>
