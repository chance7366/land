...
import { BarChart3, ExternalLink, Newspaper, Search, X } from "lucide-react";
import { AppLink as Link } from "@/components/ui/AppLink";
import { GlassCard } from "@/components/ui/GlassCard";
import {
  MOCK_RTECH_NEWS,
  matchRtechKeywords,
} from "@/lib/mockup/rtech-news-sample";
...
/** 샘플: 부동산테크 부동산뉴스 — 제목·등록일·출처(한경 집코노미)·원문, 요약·사진 없음 */
export function RtechNewsSampleClient() {
  const [keywords, setKeywords] = useState<string[]>([]);
  const [draft, setDraft] = useState("");

  const filtered = useMemo(
    () => MOCK_RTECH_NEWS.filter((row) => matchRtechKeywords(row.title, keywords)),
    [keywords],
  );
    ...
    <div className="mx-auto flex max-w-6xl flex-col gap-6 px-container-padding-mobile py-10 md:flex-row md:px-8 md:py-14">
        ...
        <GlassCard className="sticky top-24 p-3">
          ...
          <nav className="space-y-1" aria-label="뉴스 출처">
            {[
              ...
              { label: "홍성군청", count: 6, active: false, Icon: Newspaper },
              { label: "부동산테크", count: filtered.length, active: true, Icon: BarChart3 },
      ...
      <div className="min-w-0 flex-1">
        <header className="mb-6 border-b border-white/10 pb-6">
          ...
          <h1 className="mt-1 text-3xl font-extrabold text-white md:text-4xl">부동산테크</h1>
          <p className="mt-2 text-sm text-white/60">
            부동산 뉴스 · 제목·등록일 · 출처 한경 집코노미 · 원문(외부) 이동 (요약·사진 없음)
          </p>
        </header>
          ...
          <div className="mt-2 flex gap-2">
            <div className="relative min-w-0 flex-1">
              ...
              <input
                ...
                placeholder="예: 분양, 대전, 전세 — Enter로 추가"
                value={draft}
        ...
        <GlassCard className="overflow-hidden p-0">
              ...
              {filtered.map((row) => (
                <li key={row.id}>
                  <div className="flex flex-col gap-3 px-4 py-4 transition hover:bg-white/[0.03] sm:flex-row sm:items-center sm:gap-3">
                    <span className="inline-flex h-6 shrink-0 items-center whitespace-nowrap rounded-full border border-violet-400/40 bg-violet-500/15 px-2.5 text-[11px] font-bold leading-none text-violet-300">
                      R-TECH
                    </span>
                    <button
                      type="button"
          ...
          <p className="border-t border-white/10 px-4 py-3 text-center text-[11px] text-white/35">
            게시·원문 저작권은 한국부동산원 부동산테크 및 한경 집코노미에 있습니다. 제목·등록일·원문
            링크만 표시합니다.
          </p>
        </GlassCard>
        ...
        <p className="mt-4 text-center text-[11px] text-white/40">
          샘플 ·{" "}
          <a
            href="https://rtech.or.kr/portal/info/landInfo.do"
            target="_blank"
          ...
          {" · "}
          <Link href="/news?source=rtech" className="text-[#e9d5ff]/80 hover:underline">
            현재 /news?source=rtech
          </Link>
        </p>
      </div>
    </div>
