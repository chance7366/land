...
import { Building2, ExternalLink, Newspaper, Search, X } from "lucide-react";
import { AppLink as Link } from "@/components/ui/AppLink";
import { GlassCard } from "@/components/ui/GlassCard";
import {
  MOCK_CHUNGNAM_NEWS,
  matchChungnamKeywords,
} from "@/lib/mockup/chungnam-news-sample";
...
/** 샘플: 국토부와 동일 구성 — 키워드·제목·요약(첫 줄)·원문, 사진 없음, 공지 제외 */
export function ChungnamNewsSampleClient() {
  const [keywords, setKeywords] = useState<string[]>([]);
  const [draft, setDraft] = useState("");

  const filtered = useMemo(
    () =>
      MOCK_CHUNGNAM_NEWS.filter((row) =>
        matchChungnamKeywords(row.title, row.summary, keywords),
      ),
    [keywords],
  );
    ...
    <div className="mx-auto flex max-w-6xl flex-col gap-6 px-container-padding-mobile py-10 md:flex-row md:px-8 md:py-14">
        ...
        <GlassCard className="sticky top-24 p-3">
          ...
          <nav className="space-y-1" aria-label="뉴스 출처">
            {[
              ...
              { label: "충남도청", count: filtered.length, active: true, Icon: Building2 },
      ...
      <div className="min-w-0 flex-1">
        <header className="mb-6 border-b border-white/10 pb-6">
          ...
          <h1 className="mt-1 text-3xl font-extrabold text-white md:text-4xl">충남도청</h1>
          <p className="mt-2 text-sm text-white/60">
            최근 소식 · 공지 제외 · 제목·내용(첫 줄)·원문 보기 (사진 없음)
          </p>
        </header>
          ...
          <div className="mt-2 flex gap-2">
            <div className="relative min-w-0 flex-1">
              ...
              <input
                ...
                placeholder="예: 건설, 내포, 주택 — Enter로 추가"
                value={draft}
              ...
              {filtered.map((row) => (
                <li key={row.id}>
                  <div className="flex flex-col gap-3 px-4 py-4 transition hover:bg-white/[0.03] sm:flex-row sm:items-center sm:gap-3">
                    <span className="inline-flex h-6 shrink-0 items-center whitespace-nowrap rounded-full border border-amber-400/40 bg-amber-500/15 px-2.5 text-[11px] font-bold leading-none text-amber-300">
                      충남
                    </span>
                    <button
                      type="button"
        ...
        <p className="mt-4 text-center text-[11px] text-white/40">
          샘플 · 최근소식(공지 제외) · view.do 상세 ·{" "}
          <Link href="/news?source=chungnam" className="text-[#e9d5ff]/80 hover:underline">
            현재 /news?source=chungnam
          </Link>
        </p>
      </div>
    </div>
