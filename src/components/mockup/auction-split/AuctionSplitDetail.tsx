...
import type { AuctionSplitSample } from "@/lib/mockup/auction-split-sample-data";
...
type Props = {
  auction: AuctionSplitSample | null;
  onBack?: () => void;
  showBack?: boolean;
};
...
export function AuctionSplitDetail({ auction, onBack, showBack }: Props) {
  ...
  const minPct =
    auction.appraisalPrice > 0 && auction.minPrice > 0
      ? Math.round((auction.minPrice / auction.appraisalPrice) * 1000) / 10
      : null;
    ...
    <div className="space-y-3">
      ...
      <Section n={2} title="가격 · 기일">
        <div className="grid gap-2 sm:grid-cols-2">
          ...
          <Fact
            label="최저가"
            value={`${formatAuctionMoney(auction.minPrice)}${minPct != null ? ` (${minPct}%)` : ""}`}
            accent="text-[#d4af37]"
          />
          <Fact label="입찰보증금" value={formatAuctionMoney(auction.bidDeposit)} />
          <Fact label="청구금액" value={formatAuctionMoney(auction.claimAmount)} />
      ...
      <Section n={3} title="물건상세">
        <div className="grid gap-2 sm:grid-cols-2">
          {auction.formGroup === "UNIT" ? (
            ...
            <>
              ...
              {auction.formGroup !== "LAND" ? (
                <Fact label="건물면적" value={fmtArea(auction.buildingArea)} />
              ) : null}
            </>
          )}
        ...
        {auction.parcels.length > 1 ? (
          <div className="mt-3 overflow-x-auto rounded-xl border border-white/10">
            <table className="w-full min-w-[420px] text-left text-xs">
              <thead>
                <tr className="border-b border-white/10 bg-black/30 text-white/45">
                  <th className="px-3 py-2 font-semibold">목록</th>
                  <th className="px-3 py-2 font-semibold">구분</th>
                  <th className="px-3 py-2 font-semibold">소재지</th>
                  <th className="px-3 py-2 font-semibold">상세</th>
                </tr>
              </thead>
              <tbody>
                {auction.parcels.map((p) => (
                  <tr key={p.no} className="border-b border-white/5 text-white/80">
                    <td className="px-3 py-2 tabular-nums">{p.no}</td>
                    <td className="px-3 py-2">{p.listKind}</td>
                    <td className="px-3 py-2">{p.address}</td>
                    <td className="px-3 py-2 text-white/55">{p.detail || "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
