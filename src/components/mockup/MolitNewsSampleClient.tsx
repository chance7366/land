...
import { ExternalLink, Landmark, Newspaper, Search, X } from "lucide-react";
import { AppLink as Link } from "@/components/ui/AppLink";
import { GlassCard } from "@/components/ui/GlassCard";
import {
  MOCK_MOLIT_NEWS,
  matchMolitKeywords,
} from "@/lib/mockup/molit-news-sample";
...
/**
 * 샘플: 네이버부동산뉴스와 동일 구성(키워드·제목·요약·원문) — 썸네일 없음
 */
export function MolitNewsSampleClient() {
  const [keywords, setKeywords] = useState<string[]>([]);
  const [draft, setDraft] = useState("");

  const filtered = useMemo(
    () =>
      MOCK_MOLIT_NEWS.filter((row) =>
        matchMolitKeywords(row.title, row.summary, keywords),
      ),
    [keywords],
  );
    ...
    <div className="mx-auto flex max-w-6xl flex-col gap-6 px-container-padding-mobile py-10 md:flex-row md:px-8 md:py-14">
        ...
        <GlassCard className="sticky top-24 p-3">
          ...
          <nav className="space-y-1" aria-label="뉴스 출처">
            {[
              ...
              { label: "국토교통부", count: filtered.length, active: true, Icon: Landmark },
              { label: "충남도청", count: 2, active: false, Icon: Newspaper },
              { label: "홍성군청", count: 2, active: false, Icon: Newspaper },
      ...
      <div className="min-w-0 flex-1">
        <header className="mb-6 border-b border-white/10 pb-6">
          ...
          <h1 className="mt-1 text-3xl font-extrabold text-white md:text-4xl">국토교통부</h1>
          <p className="mt-2 text-sm text-white/60">
            보도자료 · 주택·토지 · 담당부서·등록일 · 원문 보기 (사진 없음)
          </p>
        </header>
          ...
          <div className="mt-2 flex gap-2">
            <div className="relative min-w-0 flex-1">
              ...
              <input
                ...
                placeholder="예: 전세, 신도시, 공공임대 — Enter로 추가"
                value={draft}
              ...
              {filtered.map((row) => (
                <li key={row.id}>
                  <div className="flex flex-col gap-3 px-4 py-4 transition hover:bg-white/[0.03] sm:flex-row sm:items-center sm:gap-3">
                    <span className="inline-flex h-6 shrink-0 items-center whitespace-nowrap rounded-full border border-sky-400/40 bg-sky-500/15 px-2.5 text-[11px] font-bold leading-none text-sky-300">
                      국토부
                    </span>
                    <button
                      type="button"
        ...
        <p className="mt-4 text-center text-[11px] text-white/40">
          샘플 · 원문 = dtl.jsp 상세 페이지 · 사진 생략 ·{" "}
          <Link href="/news?source=molit" className="text-[#e9d5ff]/80 hover:underline">
            현재 /news?source=molit
          </Link>
        </p>
      </div>
    </div>
